...
shared_ptr<Mesh> Mesh::Load_Mesh(const char* file_pass, const char* fbx_filename)
{
	...
	//ファイルから読み込み
	{
		...
		mesh_ptr->skin_meshes.resize(mesh_size);
		for (int i = 0;i < mesh_size;i++)
		{
			...
			mesh_ptr->skin_meshes[i].subsets.resize(subset_size);
			for (int j = 0;j < subset_size;j++)
			{
				getline(in_f, _s);
				mesh_ptr->skin_meshes[i].subsets[j].index_start = stoi(_s);

				getline(in_f, _s);
				mesh_ptr->skin_meshes[i].subsets[j].index_count = stoi(_s);

				in_f >> mesh_ptr->skin_meshes[i].subsets[j].diffuse.color.x >> mesh_ptr->skin_meshes[i].subsets[j].diffuse.color.y >> mesh_ptr->skin_meshes[i].subsets[j].diffuse.color.z >> mesh_ptr->skin_meshes[i].subsets[j].diffuse.color.w;
				in_f.ignore();
				...
				if (_s != "")
				{
					...
					mesh_ptr->skin_meshes[i].subsets[j].diffuse.TexPass = TexPass;
					mesh_ptr->skin_meshes[i].subsets[j].diffuse.TexName = _s;
				}
				else
				{
					mesh_ptr->skin_meshes[i].subsets[j].diffuse.TexPass = nullptr;
				}
			}
			...
			for (int row = 0; row < 4; row++)
			{
				for (int column = 0; column < 4; column++)
				{
					...
					mesh_ptr->skin_meshes[i].global_transform.m[row][column] = stof(_s);
				}
			}

			getline(in_f, _s);
			int skeletal_animation_size = stoi(_s);
			mesh_ptr->skin_meshes[i].skeletal_animation.resize(skeletal_animation_size);
			for (int j = 0;j < skeletal_animation_size;j++)
			{
				getline(in_f, _s);
				int skeletal_size = stoi(_s);
				mesh_ptr->skin_meshes[i].skeletal_animation[j].resize(skeletal_size);
				getline(in_f, _s);
				mesh_ptr->skin_meshes[i].skeletal_animation[j].sampling_time = stof(_s);
				for (int k = 0;k < skeletal_size;k++)
				{
					getline(in_f, _s);
					int bone_size = stoi(_s);
					mesh_ptr->skin_meshes[i].skeletal_animation[j].at(k).resize(bone_size);
					for (int l = 0; l < bone_size; l++)
					{
						for (int row = 0; row < 4; row++)
						{
							for (int column = 0; column < 4; column++)
							{
								...
								mesh_ptr->skin_meshes[i].skeletal_animation[j].at(k).at(l).transform.m[row][column] = stof(_s);;
							}
						}
					}
				}
			...
			// 頂点バッファの生成
			{
				...
				HRESULT hr = DxSystem::Device->CreateBuffer(&bd, &InitData, mesh_ptr->skin_meshes[i].vertex_buffer.GetAddressOf());
				assert(SUCCEEDED(hr), hr_trace(hr));
			}
			...
			// インデックスバッファの生成
			{
				...
				InitData.SysMemPitch = 0;
				InitData.SysMemSlicePitch = 0;
				HRESULT hr = DxSystem::Device->CreateBuffer(&bd, &InitData, mesh_ptr->skin_meshes[i].index_buffer.GetAddressOf());
				assert(SUCCEEDED(hr), hr_trace(hr));
			}
		}
